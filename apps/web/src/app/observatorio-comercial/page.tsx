...
import { MetricExplanation, MetricExplanationGroup } from "@/features/commercial-observatory/metric-explanation";
...
type ObservatoryData = NonNullable<ReturnType<typeof useObservatory>["data"]>;
type MetricExplanations = ObservatoryData["metricExplanations"];
...
function Seasonality({ data, explanations }: { data: ObservatoryData["seasonality"]; explanations: MetricExplanations }) {
  return (
            ...
            <Card size="sm">
              <CardHeader>
                <div className="flex items-center gap-1">
                  ...
                  <MetricExplanation explanation={explanations["seasonality.weekly_volume"]} />
                </div>
...
function Anomalies({ data, explanations }: { data: ObservatoryData["anomalies"]; explanations: MetricExplanations }) {
  return (
    <section className="flex flex-col gap-3">
      ...
      <Card size="sm">
        <CardHeader><div className="flex items-center gap-1"><CardTitle>Señales deterministas</CardTitle><MetricExplanationGroup label="Explicar señales deterministas" title="Señales deterministas" explanations={[explanations["anomalies.volume"], explanations["anomalies.conversion"]]} /></div><CardDescription>Actual {dateLabel(data.range.from)}–{inclusiveEndLabel(data.range.to)} · referencia {dateLabel(data.baseline.from)}–{inclusiveEndLabel(data.baseline.to)} · {data.rule}</CardDescription></CardHeader>
...
function Bridge({ data, range, currency, explanations }: { data: ObservatoryData["bridge"]; range: ObservatoryData["range"]; currency: string | null; explanations: MetricExplanations }) {
  const economic = data.economic;
  return (
    <section className="flex flex-col gap-3">
      <SectionTitle title="Qué cambió frente al periodo anterior" description="Descompone aritméticamente el cambio comercial y económico entre dos periodos de igual duración. Describe diferencias observadas; no atribuye causalidad." />
      <Card size="sm" aria-label="Advertencia sobre causalidad">
        <CardHeader>
          <CardTitle>Lectura descriptiva</CardTitle>
          <CardDescription>Cada contribución aritmética forma parte de un desglose que suma exactamente la diferencia observada. Es descriptivo. No demuestra causalidad.</CardDescription>
        </CardHeader>
      </Card>
      <div className="flex flex-wrap gap-2" aria-label="Contexto de la comparación">
        <Badge variant="outline">Periodo actual {dateLabel(range.from)}–{inclusiveEndLabel(range.to)}</Badge>
        <Badge variant="outline">Periodo anterior {dateLabel(data.commercial.baselineFrom)}–{inclusiveEndLabel(data.commercial.baselineTo)}</Badge>
        <Badge variant="outline">Muestra madura actual / anterior {data.commercial.current.sample} / {data.commercial.baseline.sample}</Badge>
        <Badge variant="outline">Moneda {currency ?? "no comparable"}</Badge>
      </div>
      {data.commercial.status !== "available" ? <HonestState state={data.commercial.status} /> : (
        <Card size="sm">
          <CardHeader>
            <div className="flex items-center gap-1">
              <CardTitle>Cambio de ventas</CardTitle>
              <MetricExplanationGroup
                label="Explicar cambio de ventas"
                title="Cambio de ventas"
                explanations={[
                  explanations["bridge.commercial_delta"],
                  explanations["bridge.volume_contribution"],
                  explanations["bridge.conversion_contribution"],
                ]}
              />
            </div>
            <CardDescription>Δ ventas = volumen + conversión · {data.commercial.rule}</CardDescription>
          </CardHeader>
          <CardContent>
            <dl className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4" aria-label="Desglose del cambio de ventas">
              <div><dt className="text-sm text-muted-foreground">Delta de ventas</dt><dd className="text-2xl font-semibold">{data.commercial.deltaSales.toFixed(2)}</dd></div>
              <div><dt className="text-sm text-muted-foreground">Contribución volumen</dt><dd>{data.commercial.volumeContribution.toFixed(2)}</dd></div>
              <div><dt className="text-sm text-muted-foreground">Contribución conversión</dt><dd>{data.commercial.conversionContribution.toFixed(2)}</dd></div>
              <div><dt className="text-sm text-muted-foreground">Muestra madura actual / anterior</dt><dd>{data.commercial.current.sample} / {data.commercial.baseline.sample}</dd></div>
            </dl>
          </CardContent>
        </Card>
      )}
      {economic.status !== "available" || !currency ? <HonestState state={economic.status === "available" ? "currency_required" : economic.status} /> : <Card size="sm"><CardHeader><div className="flex items-center gap-1"><CardTitle>Desglose del margen realizado</CardTitle><MetricExplanation explanation={explanations["bridge.economic_margin"]} /></div><CardDescription>{economic.rule} · Moneda resuelta por el servidor: {currency}. Sin FX implícito.</CardDescription></CardHeader><CardContent className="flex flex-col gap-3"><div className="flex flex-wrap gap-2"><Badge variant="outline">Referencia {money(economic.baseline.marginCents, currency)}</Badge><Badge variant="outline">Actual {money(economic.current.marginCents, currency)}</Badge><Badge variant="secondary">Δ {money(economic.deltaMarginCents, currency)}</Badge></div><Table><TableHeader><TableRow><TableHead className="h-8">Contribución aritmética</TableHead><TableHead className="h-8">Importe</TableHead></TableRow></TableHeader><TableBody>{economic.contributions.map((row) => <TableRow className="h-9" key={row.key}><TableCell className="px-2 py-1">{row.label}</TableCell><TableCell className="px-2 py-1">{money(row.amountCents, currency)}</TableCell></TableRow>)}</TableBody></Table></CardContent></Card>}
    </section>
  );
}
...
function Risk({ data, explanations }: { data: ObservatoryData["risk"]; explanations: MetricExplanations }) {
      ...
      {data.status !== "available" ? <HonestState state={data.status} /> : (
        <>
          <div className="grid gap-3 md:grid-cols-2">
            <Card size="sm"><CardHeader><div className="flex items-center gap-1"><CardTitle>Cobertura</CardTitle><MetricExplanationGroup label="Explicar cobertura" title="Cobertura" explanations={[explanations["quality.without_attribution"], explanations["quality.sales_without_ledger"]]} /></div></CardHeader><CardContent className="grid gap-2 sm:grid-cols-3"><div><p className="text-xs text-muted-foreground">Leads</p><p>n={data.coverage.sample}</p></div><div><p className="text-xs text-muted-foreground">Sin atribución</p><p>{data.coverage.withoutAttribution}</p></div><div><p className="text-xs text-muted-foreground">Ventas sin registro económico</p><p>{data.coverage.salesWithoutLedger === null ? "No evaluable sin moneda" : data.coverage.salesWithoutLedger}</p></div></CardContent></Card>
          ...
          <Card size="sm">
            <CardHeader><div className="flex items-center gap-1"><CardTitle>Concentración por dimensión</CardTitle><MetricExplanationGroup label="Explicar concentración por dimensión" title="Concentración por dimensión" explanations={[explanations["risk.top1"], explanations["risk.top3"], explanations["risk.hhi"], explanations["risk.absolute_exposure"], explanations["risk.negative_margin"]]} /></div><CardDescription>{dateLabel(data.range.from)}–{inclusiveEndLabel(data.range.to)} · {data.rule}</CardDescription></CardHeader>
  ...
  return (
    <section className="flex flex-col gap-4">
      ...
      {query.isPending ? <div className="flex flex-col gap-3"><p className="text-sm text-muted-foreground">Cargando observatorio…</p><Skeleton className="h-56 w-full" /><Skeleton className="h-40 w-full" /></div> : query.isError ? <Empty heading="No se pudo cargar" description="No se muestran cálculos parciales; revisa el rango y vuelve a intentarlo." /> : query.data.coverage.observations === 0 ? <Empty heading="Sin observaciones" description="No hay observaciones controladas por el servidor dentro del histórico disponible." /> : (
        <>
          <div className="flex flex-wrap items-center gap-2"><Badge variant="outline">{query.data.policyVersion}</Badge><Badge variant="outline">{query.data.metricExplanationPolicyVersion}</Badge><Badge variant="outline">Europe/Madrid</Badge><Badge variant="secondary">{query.data.coverage.observations} leads únicos</Badge><MetricExplanation explanation={query.data.metricExplanations["quality.unique_observations"]} />{query.data.resolvedCurrency ? <Badge variant="outline">Moneda única: {query.data.resolvedCurrency}</Badge> : query.data.currencies.length > 1 ? <Badge variant="outline">{query.data.currencies.length} monedas separadas</Badge> : null}{query.data.coverage.duplicateObservationsExcluded > 0 ? <><Badge variant="outline">{query.data.coverage.duplicateObservationsExcluded} duplicados excluidos</Badge><MetricExplanation explanation={query.data.metricExplanations["quality.duplicates_excluded"]} /></> : null}</div>
          <Tabs defaultValue="seasonality">
            <TabsList className="h-auto w-fit max-w-full flex-nowrap justify-start gap-1 rounded-lg border bg-muted/40 p-1" aria-label="Análisis del observatorio"><TabsTrigger className="h-11! min-h-11! flex-none rounded-md px-3 py-2 text-sm data-active:bg-background" value="seasonality">Estacionalidad</TabsTrigger><TabsTrigger className="h-11! min-h-11! flex-none rounded-md px-3 py-2 text-sm data-active:bg-background" value="anomalies">Radar de anomalías</TabsTrigger><TabsTrigger className="h-11! min-h-11! flex-none rounded-md px-3 py-2 text-sm data-active:bg-background" value="bridge">Qué cambió frente al periodo anterior</TabsTrigger><TabsTrigger className="h-11! min-h-11! flex-none rounded-md px-3 py-2 text-sm data-active:bg-background" value="risk">Mapa de riesgo</TabsTrigger></TabsList>
            <TabsContent value="seasonality"><Seasonality data={query.data.seasonality} explanations={query.data.metricExplanations} /></TabsContent>
            <TabsContent value="anomalies"><Anomalies data={query.data.anomalies} explanations={query.data.metricExplanations} /></TabsContent>
            <TabsContent value="bridge"><Bridge data={query.data.bridge} range={query.data.range} currency={query.data.resolvedCurrency} explanations={query.data.metricExplanations} /></TabsContent>
            <TabsContent value="risk"><Risk data={query.data.risk} explanations={query.data.metricExplanations} /></TabsContent>
          </Tabs>
        </>
      )}
    </section>
