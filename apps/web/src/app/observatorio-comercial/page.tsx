...
import { MetricExplanation } from "@/features/commercial-observatory/metric-explanation";
...
function Anomalies({ data, explanations }: { data: ObservatoryData["anomalies"]; explanations: MetricExplanations }) {
  return (
    <section className="flex flex-col gap-3">
      ...
      <Card size="sm">
        <CardHeader><div className="flex items-center gap-1"><CardTitle>Señales deterministas</CardTitle><MetricExplanation explanation={explanations["anomalies.volume"]} /><MetricExplanation explanation={explanations["anomalies.conversion"]} /></div><CardDescription>Actual {dateLabel(data.range.from)}–{inclusiveEndLabel(data.range.to)} · referencia {dateLabel(data.baseline.from)}–{inclusiveEndLabel(data.baseline.to)} · {data.rule}</CardDescription></CardHeader>
...
function Bridge({ data, currency, explanations }: { data: ObservatoryData["bridge"]; currency: string | null; explanations: MetricExplanations }) {
      ...
      {data.status !== "available" ? <HonestState state={data.status} /> : (
        <>
          <div className="grid gap-3 md:grid-cols-2">
            <Card size="sm"><CardHeader><div className="flex items-center gap-1"><CardTitle>Cobertura</CardTitle><MetricExplanation explanation={explanations["quality.without_attribution"]} /><MetricExplanation explanation={explanations["quality.sales_without_ledger"]} /></div></CardHeader><CardContent className="grid gap-2 sm:grid-cols-3"><div><p className="text-xs text-muted-foreground">Leads</p><p>n={data.coverage.sample}</p></div><div><p className="text-xs text-muted-foreground">Sin atribución</p><p>{data.coverage.withoutAttribution}</p></div><div><p className="text-xs text-muted-foreground">Ventas sin registro económico</p><p>{data.coverage.salesWithoutLedger === null ? "No evaluable sin moneda" : data.coverage.salesWithoutLedger}</p></div></CardContent></Card>
          ...
          <Card size="sm">
            <CardHeader><div className="flex items-center gap-1"><CardTitle>Concentración por dimensión</CardTitle><MetricExplanation explanation={explanations["risk.top1"]} /><MetricExplanation explanation={explanations["risk.top3"]} /><MetricExplanation explanation={explanations["risk.hhi"]} /><MetricExplanation explanation={explanations["risk.absolute_exposure"]} /><MetricExplanation explanation={explanations["risk.negative_margin"]} /></div><CardDescription>{dateLabel(data.range.from)}–{inclusiveEndLabel(data.range.to)} · {data.rule}</CardDescription></CardHeader>
      ...
      {query.isPending ? <div className="flex flex-col gap-3"><p className="text-sm text-muted-foreground">Cargando observatorio…</p><Skeleton className="h-56 w-full" /><Skeleton className="h-40 w-full" /></div> : query.isError ? <Empty heading="No se pudo cargar" description="No se muestran cálculos parciales; revisa el rango y vuelve a intentarlo." /> : query.data.coverage.observations === 0 ? <Empty heading="Sin observaciones" description="No hay observaciones controladas por el servidor dentro del histórico disponible." /> : (
        <>
          ...
          <Tabs defaultValue="seasonality">
            ...
            <TabsContent value="bridge"><Bridge data={query.data.bridge} currency={query.data.resolvedCurrency} explanations={query.data.metricExplanations} /></TabsContent>
