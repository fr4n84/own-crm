...
describe("commercial observatory page", () => {
  it("renders all four explainable sections and honest UI states", () => {
    for (const label of ["Observatorio comercial", "Estacionalidad", "Radar de anomalías", "Qué cambió frente al periodo anterior", "Mapa de dependencia y riesgo"]) expect(source).toContain(label);
    ...
    expect(source).toContain("No demuestra causalidad");
    expect(source).not.toMatch(/\bbarras?\b/iu);
