...
export const emailMarketingRouter = router({
  ...
  buildAudience: adminProcedure.input(z.object({ campaignId })).mutation(({ ctx, input }) => mapServiceError(() => emailMarketingService.buildAudience({ ...input, actorId: ctx.session.user.id }))),
